...
public class Peach extends Entity implements Exeggutor, Movers {
    ...
    public Peach(String id, Point position, List<PImage> images, double animationPeriod, double actionPeriod) {
    ...
    public Point nextPositionPeach(WorldModel world, Point destPos) {
...
// some sort of predicate that returns a boolean
    ...
    public boolean moveTo(WorldModel world, Entity target, EventScheduler scheduler) {
        if (this.getPosition().adjacent(target.getPosition())) {
            ...
            Point nextPos = nextPositionPeach(world, target.getPosition());
    ...
    public void executeActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Entity> peachOpp = findNearest(world, this.getPosition(), new ArrayList<>(List.of(Bowser.class)));

        if (peachOpp.isPresent()) {
            Point tgtPos = peachOpp.get().getPosition();

            if (moveTo(world, peachOpp.get(), scheduler)) {
//     public Sapling(String id, Point position, List<PImage> images, int health, double actionPeriod){
                Entity dude = new DudeNotFull(world.getDudeKey()+ "_" + peachOpp.get().getId(), tgtPos, imageStore.getImageList(world.getDudeKey()), 4, 0.7, 0.3, 1);
