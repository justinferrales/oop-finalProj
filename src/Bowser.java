...
public class Bowser extends Entity implements Exeggutor, Movers{
    ...
    public Bowser(String id, Point position, List<PImage> images, double animationPeriod, double actionPeriod){
    ...
    public Point nextPositionBowser(WorldModel world, Point destPos) {
...
// some sort of predicate that returns a boolean
    ...
    public boolean moveTo(WorldModel world, Entity target, EventScheduler scheduler) {
        if (this.getPosition().adjacent(target.getPosition())) {
            ...
            Point nextPos = nextPositionBowser(world, target.getPosition());
    ...
    public void executeActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Entity> bowserOpp = findNearest(world, this.getPosition(), new ArrayList<>(List.of(DudeNotFull.class, DudeFull.class )));

        if (bowserOpp.isPresent()) {
            Point tgtPos = bowserOpp.get().getPosition();

            if (moveTo(world, bowserOpp.get(), scheduler)) {
//     public Sapling(String id, Point position, List<PImage> images, int health, double actionPeriod){
                Entity Peach = new Peach(world.getPeachKey() + "_" + bowserOpp.get().getId(), tgtPos, imageStore.getImageList(world.getPeachKey()), 0.9,0.3);
                world.addEntity(Peach);
                Peach.scheduleActions(scheduler, world, imageStore);
            }
        }
